<%@ WebHandler Language="VB" Class="WsGetReport" %>
...
Public Class WsGetReport : Implements IHttpHandler
    ...
    Public Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        Dim Debug, RETURN_PAGE, DOMAIN, DMID, REP_ID, AccessToken As String
        ...
        mydebuglog = log4net.LogManager.GetLogger("GetReportDebugLog")
        ...
        Dim DMS As New com.certegrity.cloudsvc.dms.Service
        
        ' Variable declarations
        Dim errmsg As String                    ' Error message (if any)
        Dim MSG_ID, ATTACH_DOC_ID, ATTACHMENT, ATTACH_TYPE As String
        Dim RFileName, RItemName, RDescription, Rext As String
        Dim EOL, LaunchProtocol, ReturnDestination, NextLink, outdata, ErrLvl As String
        Dim NewParticipation As Boolean
        Dim MSG1, CRSE_NAME, temp, HLINK, CheckToken As String
        ...
        Dim DOC_FN, VERSION_ID, dName, newwindow, FileName, ReDirect, PublicKey, ItemName, EMAIL_ADDR As String
        ...
        Try
            ...
            tempdebug = System.Configuration.ConfigurationManager.AppSettings.Get("GetReport_debug")
        ...
        If Logging = "Y" Then
            logfile = "C:\Logs\WsGetReport.log"
            Try
                log4net.GlobalContext.Properties("GetReportLogFileName") = logfile
        ...
        If Not context.Request.QueryString("ID") Is Nothing Then
            REP_ID = context.Request.QueryString("ID")
        Else
            REP_ID = "643YRJ4IM6V0"
            AccessToken = "57PEUAIK"
            DOMAIN = "TIPS"
            UID = "NA49O5OAO1"
            cookieid = "NA49O5OAO1"
            SessID = "S22142151GPXG"
            CURRENT_PAGE = "https://www.gettips.com/mobile/OpenReport.html?ID=643YRJ4IM6V0&PP=TIPS&TK=57PEUAIK&UID=NA49O5OAO1&HP=gettips.com&SES=S22142151GPXG&PROT=https:&CUR=https://www.gettips.com/mobile/index.html&GOOG=Y"
            callback = "jQuery112404936563050139666_1558014809096&_=1558014809098"
        End If

        If Not context.Request.QueryString("DMID") Is Nothing Then
            DMID = context.Request.QueryString("DMID")
        End If
        ...
        If Not context.Request.QueryString("TK") Is Nothing Then
            AccessToken = context.Request.QueryString("TK")
        End If
        ...
        If Debug = "Y" Then
            ...
            mydebuglog.Debug("  REP_ID: " & REP_ID)
            mydebuglog.Debug("  DMID: " & DMID)            
            mydebuglog.Debug("  AccessToken: " & AccessToken)
        ...
        If DMID = "" And REP_ID = "" Then GoTo AvailError
        ...
        If Not cmd Is Nothing Then
            ...
            If UID <> "" And SessID <> "" Then
                SqlS = "SELECT TOP 1 (SELECT CASE WHEN H.LOGOUT_DT IS NULL THEN 'Y' ELSE 'N' END) AS LOGGED_IN, SC.CON_ID, " & _
                    "SC.SUB_ID, C.PR_DEPT_OU_ID, S.DOMAIN, C.X_PR_LANG_CD, SC.TOKEN, C.EMAIL_ADDR " & _
                ...
                Try
                    ...
                    If Not dr Is Nothing Then
                        While dr.Read()
                            ...
                            CheckToken = Trim(CheckDBNull(dr(6), enumObjectType.StrType))
                            EMAIL_ADDR = Trim(CheckDBNull(dr(7), enumObjectType.StrType))
                        End While
                    End If
                Catch ex As Exception
                    GoTo AccessError
                ...
                dr.Close()
                If Debug = "Y" Then
                    mydebuglog.Debug("  .. CheckToken: " & CheckToken)
                    mydebuglog.Debug("  .. LOGGED_IN: " & LOGGED_IN)
                    mydebuglog.Debug("  .. CONTACT_ID: " & CONTACT_ID)
                    mydebuglog.Debug("  .. LANG_CD: " & LANG_CD)
                    mydebuglog.Debug("  .. USER_DOMAIN: " & USER_DOMAIN)
                    mydebuglog.Debug("  .. EMAIL_ADDR: " & EMAIL_ADDR)
                End If
                If LOGGED_IN <> "Y" Then GoTo AccessError
                If CheckToken <> AccessToken Then GoTo AccessError
            Else
                GoTo AccessError
            End If
            
            ' ================================================
            ' CONVERT REPORT ID INTO DOCUMENT            
            If REP_ID <> "" And DMID = "" Then
                If IsNumeric(REP_ID) Then
                    ' If the report id is numeric, assume it is a document id
                    DMID = REP_ID
                Else
                    If EMAIL_ADDR <> "" Then
                        ' Locate the message to get details
                        SqlS = "SELECT MS_IDENT, ATTACH_DOC_ID, ATTACH_TYPE, ATTACHMENT " & _
                               "FROM scanner.dbo.MESSAGES " & _
                               "WHERE SEND_TO='" & EMAIL_ADDR & "' AND ATTACHMENT LIKE '%" & REP_ID & "%'"
                        If Debug = "Y" Then mydebuglog.Debug(vbCrLf & "  Get the Message: " & vbCrLf & "  " & SqlS)
                        Try
                            cmd.CommandText = SqlS
                            dr = cmd.ExecuteReader()
                            If Not dr Is Nothing Then
                                While dr.Read()
                                    MSG_ID = Trim(Str(CheckDBNull(dr(0), enumObjectType.IntType)))
                                    ATTACH_DOC_ID = Trim(CheckDBNull(dr(1), enumObjectType.StrType))
                                    ATTACH_TYPE = Trim(CheckDBNull(dr(2), enumObjectType.StrType))
                                    ATTACHMENT = Trim(CheckDBNull(dr(3), enumObjectType.StrType))
                                End While
                            End If
                        Catch ex As Exception
                            GoTo AccessError
                        ...
                        dr.Close()
                        If Debug = "Y" Then
                            mydebuglog.Debug("  .. MSG_ID: " & MSG_ID)
                            mydebuglog.Debug("  .. ATTACH_DOC_ID: " & ATTACH_DOC_ID)
                            mydebuglog.Debug("  .. ATTACHMENT: " & ATTACHMENT)
                        End If                        
                    End If

                    ' If ATTACH_DOC_ID is blank then convert to a DMS document
                    If ATTACH_DOC_ID = "" Then
                        ' Get Details from the report                        
                        SqlS = "SELECT '" & REP_ID & ".'+lower(S.FORMAT) AS RFILENAME, R.NAME, R.DESCRIPTION, S.FORMAT " & _
                               ...
                               "WHERE S.ROW_ID='" & REP_ID & "'"
                        If Debug = "Y" Then mydebuglog.Debug(vbCrLf & "  Get the Report: " & vbCrLf & "  " & SqlS)
                        Try
                            cmd.CommandText = SqlS
                            dr = cmd.ExecuteReader()
                            If Not dr Is Nothing Then
                                While dr.Read()
                                    RFileName = Trim(CheckDBNull(dr(0), enumObjectType.StrType))
                                    RItemName = Trim(CheckDBNull(dr(1), enumObjectType.StrType))
                                    If RItemName = "" Then RItemName = "Online Training Receipt"
                                    RDescription = Trim(CheckDBNull(dr(2), enumObjectType.StrType))
                                    If RDescription = "" Then RDescription = "This is a receipt for your recent eTIPS training class"
                                    Rext = Trim(CheckDBNull(dr(3), enumObjectType.StrType))
                                End While
                            End If
                        Catch ex As Exception
                            GoTo AccessError
                        ...
                        dr.Close()
                        If Debug = "Y" Then
                            mydebuglog.Debug("  .. RFileName: " & RFileName)
                            mydebuglog.Debug("  .. RItemName: " & RItemName)
                            mydebuglog.Debug("  .. RDescription: " & RDescription)
                            mydebuglog.Debug("  .. Rext: " & Rext)
                        End If
                        
                        ' Generate DMS document
                        Dim results As String
                        results = DMS.SaveDMSDoc("", "", REP_ID, RItemName, RFileName, "", RDescription, Rext, "", DOMAIN, "R", "Y", UID, CONTACT_ID, "REDO", "Y", SUB_ID, "", "", Debug)
                        If Debug = "Y" Then
                            mydebuglog.Debug("  .. results: " & results)
                        End If
                        If IsNumeric(results) Then
                            DMID = results
                            
                            ' Update the message so that the converted document can be found again
                            If MSG_ID <> "" And DMID <> "" Then
                                SqlS = "UPDATE scanner.dbo.MESSAGES SET ATTACH_DOC_ID='" & DMID & "', READ_FLG='Y' WHERE MS_IDENT=" & MSG_ID
                                If Debug = "Y" Then mydebuglog.Debug(vbCrLf & "  Update ATTACH_DOC_ID: " & vbCrLf & "  " & SqlS)
                                temp = ExecQuery("Update", "MESSAGES", cmd, SqlS, mydebuglog, "N")
                            End If
                        End If
                    Else
                        DMID = ATTACH_DOC_ID
            ...
            If DMID <> "" Then
                If UID = "" Then
                    SqlS = "SELECT D.dfilename, V.version, D.name " & _
                ...
                Else
                    ...
                    SqlS = "SELECT D.dfilename, V.version, D.name " & _
                        ...
                        "LEFT OUTER JOIN DMS.dbo.Groups G1 ON G1.row_id=UA.access_id AND UA.type_id='G' AND G1.type_cd='Domain' " & _
                        "LEFT OUTER JOIN DMS.dbo.Category_Access CA ON CA.cat_id=C.row_id AND CA.user_access_id=UA.row_id " & _
                        "WHERE D.row_id='" & DMID & "' AND D.deleted IS NULL AND " & temp & " AND " & _
                        "(CHARINDEX('P',CA.access_type)>0 OR CHARINDEX('R',DU.access_type)>0) " & _
                        "GROUP BY D.dfilename, V.version, D.name"
                End If
            ...
            If InStr(ItemName, "getti.ps") > 0 Then
                ReDirect = ItemName
            Else
                ReDirect = "http://hciscorm.certegrity.com/media/GetDImage.ashx?Domain=" & USER_DOMAIN & "&PublicKey=" & PublicKey & "&ItemName=" & ItemName & "&Debug=Y"
            End If
            If Debug = "Y" Then
...
CloseOut2:
        ...
        If Trim(errmsg) <> "" Then myeventlog.Error("WsGetReport.ashx: " & ErrLvl & ": " & Trim(errmsg))
        myeventlog.Info("WsGetReport.ashx : DMID : " & DMID & ", UID: " & UID & ", SessID: " & SessID)
