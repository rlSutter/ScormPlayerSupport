<%@ WebHandler Language="VB" Class="WsLoginT" %>
...
Public Class WsLoginT : Implements IHttpHandler
    ...
    Public Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        mydebuglog = log4net.LogManager.GetLogger("WsLoginTDebugLog")
        ...
        Dim TERM_FLG, SUB_CON_ID, SVC_TYPE, CON_ID, TRAINING_ACCESS, TRAINER_ACC_FLG, PAID_USER_FLG As String
        ...
        Try
            ...
            tempdebug = System.Configuration.ConfigurationManager.AppSettings.Get("WsLoginT_debug")
        ...
        If Logging = "Y" Then
            logfile = "C:\Logs\WsLoginT.log"
            Try
                log4net.GlobalContext.Properties("WsLoginTLogFileName") = logfile
        ...
        If Not context.Request.QueryString("RNL") Is Nothing Then
            LOGIN_REDIRECT = UCase(context.Request.QueryString("RNL"))
        End If
        
        If Not context.Request.QueryString("RID") Is Nothing Then
            REDIRECT_REC = UCase(context.Request.QueryString("RID"))
        End If
        
        If Not context.Request.QueryString("AID") Is Nothing Then
            ALT_RECORD_ID = UCase(context.Request.QueryString("AID"))
        End If
        
        If Not context.Request.QueryString("RCN") Is Nothing Then
            REF_CON_ID = UCase(context.Request.QueryString("RCN"))
        End If
...
ComputeNext:
                ...
                Select Case DOMAIN
                    Case "TIPS"
                        If InStr(1, ReDirect, "&") = 0 Then
                            ReDirect = "https://web.gettips.com/plogini.nsf/pWsLogin?OpenAgent&ID=" & sUserID & "&SESS=" & SessionID & "&RNL=" & ReDirect & "&REF=certegrity.com&DOM=" & DOMAIN
                        Else
            ...
            Else
                If LOGIN_REDIRECT <> "" Then
                    If InStr(1, LOGIN_REDIRECT, "&RNL") = 0 Then
                        ReDirect = ReDirect & "&RNL=" & LOGIN_REDIRECT
                    Else
            ...
            If SVC_TYPE = "" Then Student = False
            ...
            ' Process redirects
            If Student Then
                If ReDirect = Path & "/Main?OpenForm&PP=" & DOMAIN Then
                    If LANG_CD <> "ENU" Then
                ...
                If LCase(LOGIN_REDIRECT) = "invoice" Then
                    If LANG_CD <> "ENU" Then
                        mReDirect = "https://www.gettips.com/mobile/" & LANG_CD & "/index.html?UID=" & sUserID & "&SES=" & SessionID & "&OID=" & REDIRECT_REC & "&PP=" & DOMAIN & "#ord"
                    Else
                        mReDirect = "https://www.gettips.com/mobile/index.html?UID=" & sUserID & "&SES=" & SessionID & "&OID=" & REDIRECT_REC & "&PP=" & DOMAIN & "#ord"
                    End If
                    Mobile = True
                    GoTo ExitmCM
                End If
                If LCase(LOGIN_REDIRECT) = "cardaddr" Then
                    If LANG_CD <> "ENU" Then
                        mReDirect = "https://www.gettips.com/mobile/" & LANG_CD & "/index.html?UID=" & sUserID & "&SES=" & SessionID & "&PP=" & DOMAIN & "#requpdcard"
                    Else
                        mReDirect = "https://www.gettips.com/mobile/index.html?UID=" & sUserID & "&SES=" & SessionID & "&PP=" & DOMAIN & "#requpdcard"
                    End If
                    Mobile = True
                    GoTo ExitmCM
                End If
                If LCase(LOGIN_REDIRECT) = "partrec" Then
                    If LANG_CD <> "ENU" Then
                        mReDirect = "https://www.gettips.com/mobile/" & LANG_CD & "/index.html?UID=" & sUserID & "&SES=" & SessionID & "&PD=" & REDIRECT_REC & "&PP=" & DOMAIN & "#cert"
                    Else
                        mReDirect = "https://www.gettips.com/mobile/index.html?UID=" & sUserID & "&SES=" & SessionID & "&PD=" & REDIRECT_REC & "&PP=" & DOMAIN & "#cert"
                    End If
                    Mobile = True
                    GoTo ExitmCM
                End If
                ...
                If LCase(LOGIN_REDIRECT) = "orderpass" Then
                    ...
                    GoTo ExitmCM
                End If
                If LCase(LOGIN_REDIRECT) = "listreg" And SVC_TYPE = "PUBLIC ACCESS" Then
                    If LANG_CD <> "ENU" Then
                        mReDirect = "https://www.gettips.com/mobile/" & LANG_CD & "/index.html?UID=" & sUserID & "&SES=" & SessionID & "&PP=" & DOMAIN
                    Else
                        mReDirect = "https://www.gettips.com/mobile/index.html?UID=" & sUserID & "&SES=" & SessionID & "&PP=" & DOMAIN
                ...
                If LCase(LOGIN_REDIRECT) = "mydocs" Then
                    If LANG_CD <> "ENU" Then
                        mReDirect = "https://www.gettips.com/mobile/" & LANG_CD & "/index.html?UID=" & sUserID & "&SES=" & SessionID & "&PP=" & DOMAIN & "#docs"
                    Else
                        mReDirect = "https://www.gettips.com/mobile/index.html?UID=" & sUserID & "&SES=" & SessionID & "&PP=" & DOMAIN & "#docs"
                    End If
                    Mobile = True
                    GoTo ExitmCM
                End If
                ...
                If LCase(LOGIN_REDIRECT) = "newsurvey" Then
                    If LANG_CD <> "ENU" Then
                        mReDirect = "https://www.gettips.com/mobile/" & LANG_CD & "/OpenSurvey.html?UID=" & sUserID & "&SES=" & SessionID & "&CON=" & CON_ID & "&ID=" & REDIRECT_REC & "&PP=" & DOMAIN & "&PROT=https:&CUR=https://www.gettips.com/mobile/index.html"
                    Else
                        mReDirect = "https://www.gettips.com/mobile/OpenSurvey.html?UID=" & sUserID & "&SES=" & SessionID & "&CON=" & CON_ID & "&ID=" & REDIRECT_REC & "&PP=" & DOMAIN & "&PROT=https:&CUR=https://www.gettips.com/mobile/index.html"
                    End If
                    Mobile = True
                    GoTo ExitmCM
                End If
                If LCase(LOGIN_REDIRECT) = "allsurveylist" Then
                    If LANG_CD <> "ENU" Then
                        mReDirect = "https://www.gettips.com/mobile/" & LANG_CD & "/index.html?UID=" & sUserID & "&SES=" & SessionID & "&PP=" & DOMAIN & "#cert"
                    Else
                        mReDirect = "https://www.gettips.com/mobile/index.html?UID=" & sUserID & "&SES=" & SessionID & "&PP=" & DOMAIN & "#cert"
                    End If
                    Mobile = True
                    GoTo ExitmCM
                End If
                If LCase(LOGIN_REDIRECT) = "surveylist" Then
                    If LANG_CD <> "ENU" Then
                        mReDirect = "https://www.gettips.com/mobile/" & LANG_CD & "/index.html?SVL=Y&UID=" & sUserID & "&SES=" & SessionID & "&PP=" & DOMAIN & "#cert"
                    Else
                        mReDirect = "https://www.gettips.com/mobile/index.html?SVL=Y&UID=" & sUserID & "&SES=" & SessionID & "&PP=" & DOMAIN & "#cert"
                    End If
                    Mobile = True
                    GoTo ExitmCM
                End If
...
ExitmCM:
                ...
                If SUB_ID <> "" Then
                    ...
                    If sUserID <> "" Then
                        SqlS = "INSERT INTO reports.dbo.CM_LOG(REG_ID, SESSION_ID, ACTION, REMOTE_ADDR, BROWSER) " & _
                               "VALUES('" & sUserID & "','" & SessionID & "','PWSLOGINT.ASHX LOGIN', '" & REMOTE_ADDR & "','" & BROWSER & "')"
            ...
            output = output & "}"
            output = output & "</style>"
            output = output & "<!--Flash Check-->" & EOL & "<script src=""https://www.gettips.com/js/swfobject.js"" language=""JavaScript"" type=""text/javascript""></script>" & EOL & _
            ...
            output = output & "function openNewWindow(fileName,theWidth,theHeight) {" & EOL & _
            ...
            "      var mobileflg = " & LCase(Mobile) & ";" & EOL & _
            "      check = mobileAndTabletcheck();" & EOL & _
            "      if (check && home=='home' && lredirect=='' && !mobileflg) {" & EOL & _
...
CloseOut2:
        ...
        If Trim(errmsg) <> "" Then myeventlog.Error("WsLoginT.ashx : " & ErrLvl & ": " & Trim(errmsg))
        myeventlog.Info("WsLoginT.ashx : sUserID: " & sUserID & ", SessionID: " & SessionID & ", ReDirect: " & ReDirect)
        ...
        If Debug <> "T" Then
            ...
            Try
                LoggingService.LogPerformanceDataAsync(System.Environment.MachineName.ToString, "WsLoginT", LogStartTime, VersionNum, Debug)
            Catch ex As Exception
            End Try
        End If
